...
#210000 images for fine-tuning
TRAIN_POS_NUM = 20000
TRAIN_NEG_NUM = 1200000

#21048 images for testing
TEST_POS_NUM = 9376
TEST_NEG_NUM = 604176
...
There is no file both in test.txt and train.txt
"""
def prepare_dataset(data_path, train_pos_num, train_neg_num, test_pos_num, test_neg_num):
    os.chdir(data_path)   
    neg_list = os.listdir(data_path + 'neg')
    pos_list = os.listdir(data_path + 'pos')
    neg_cnt = len(neg_list)
    pos_cnt = len(pos_list)
    neg_num_list = range(0,neg_cnt)
    pos_num_list = range(0,pos_cnt)
        
    with open('train.txt', 'w') as f:
        pos_sample = random.sample(pos_num_list, train_pos_num)
        neg_sample = random.sample(neg_num_list, train_neg_num)
        for i in range(train_pos_num):
            f.write(data_path + 'pos/' + pos_list[pos_sample[i]] + ' ' + '1' + '\n')
            pos_num_list.remove(pos_sample[i])
        for i in range(train_neg_num):
            f.write(data_path + 'neg/' + neg_list[neg_sample[i]] + ' ' + '0' + '\n')
            neg_num_list.remove(neg_sample[i])
        
    with open('test.txt', 'w') as f:
        pos_sample = random.sample(pos_num_list, test_pos_num)
        neg_sample = random.sample(neg_num_list, test_neg_num)
        for i in range(test_pos_num):
            f.write(data_path + 'pos/' + pos_list[pos_sample[i]] + ' ' + '1' + '\n')
        for i in range(test_neg_num):
            f.write(data_path + 'neg/' + neg_list[neg_sample[i]] + ' ' + '0' + '\n')

    print "train.txt and test.txt are saved on: %s" %data_path
...
def solver(train_net_path, test_net_path=None, base_lr=0.001):
    ...
    s.snapshot = 1000 
    s.snapshot_prefix = caffe_root + 'models/objdet/objdet'
...
if __name__ == '__main__':
    ...
    #prepare_dataset(data_path, TRAIN_POS_NUM, TRAIN_NEG_NUM, TEST_POS_NUM, TEST_NEG_NUM)
