...
import sys
import os
caffe_root = '/home/intel/workspace/caffe/'
...
data_path = '/home/intel/workspace/dataset/Defect/'
...
TODO: make sure no same items in training list and testing list
"""
def prepare_dataset(data_path, train_pos_num, train_neg_num, test_pos_num, test_neg_num):
    ...
    with open('train.txt', 'w') as f:
        for i in range(train_pos_num):
            f.write(data_path + 'pos/' + pos_list[np.random.randint(pos_cnt)] + ' ' + '1' + '\n')
        for i in range(train_neg_num):
            f.write(data_path + 'neg/' + neg_list[np.random.randint(neg_cnt)] + ' ' + '0' + '\n')
...
def detect_net(train=True, learn_all=False, subset=None):
    if subset is None:
        subset = 'train' if train else 'test'
    source = caffe_root + 'data/defect/%s.txt' % subset
...
def solver(train_net_path, test_net_path=None, base_lr=0.001):
    ...
    s.snapshot = 10000 
    s.snapshot_prefix = caffe_root + 'models/defect_detection/defect_detection'
...
def eval_detect_net(weights, test_iters=1000):
    test_net = caffe.Net(detect_net(train=False), weights, caffe.TEST)
    accuracy = 0
